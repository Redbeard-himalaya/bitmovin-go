...
func main() {
	...
	cloudRegions := []bitmovintypes.CloudRegion {
		bitmovintypes.CloudRegionAWSUSEast1,
		bitmovintypes.CloudRegionAWSUSWest1,
		bitmovintypes.CloudRegionAWSUSWest2,
		bitmovintypes.CloudRegionAWSEUWest1,
		bitmovintypes.CloudRegionAWSEUCentral1,
		bitmovintypes.CloudRegionAWSAPSouth1,
		bitmovintypes.CloudRegionAWSAPNortheast1,
		bitmovintypes.CloudRegionAWSAPNortheast2,
		bitmovintypes.CloudRegionAWSAPSoutheast1,
		bitmovintypes.CloudRegionAWSAPSoutheast2,
		bitmovintypes.CloudRegionAWSSAEast1,
		bitmovintypes.CloudRegionGoogleEuropeWest1,
		bitmovintypes.CloudRegionGoogleUSEast1,
		bitmovintypes.CloudRegionGoogleUSCentral1,
		bitmovintypes.CloudRegionGoogleAsiaEast1,
		bitmovintypes.CloudRegionExternal,
	}

	for _, cloudRegion := range cloudRegions {
		wg.Add(1)
		go CloudRegionTest(cloudRegion, wg)
	}
	wg.Wait()
}

func CloudRegionTest(cloudRegion bitmovintypes.CloudRegion, wg sync.WaitGroup) {
	fmt.Printf("CloudRegionTest: %s\n", cloudRegion)
	defer wg.Done()
	...
	httpInput := &models.HTTPInput{
		Host: stringToPtr("3.88.204.27:10081"),
	}
	...
	s3Output := &models.S3Output{
		AccessKey:   stringToPtr("AKIAS3ZJEFTTAY4IEM7W"),
		SecretKey:   stringToPtr("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"),
		BucketName:  stringToPtr("media.getter-dev.tk"),
	...
	encoding := &models.Encoding{
		Name:        stringToPtr((string)("rdb cloud region test " + cloudRegion)),
	...
	videoInputStream := models.InputStream{
		...
		InputPath:     stringToPtr("rdb_test/HD720x1280_30fps.MOV"),
	...
	videoMuxing1080pOutput := models.Output{
		...
		OutputPath: stringToPtr((string)("rdb_test/" + cloudRegion + "/video/1080p")),
		ACL:        acl,
	}
	videoMuxing720pOutput := models.Output{
		OutputID:   s3OutputResp.Data.Result.ID,
		OutputPath: stringToPtr((string)("rdb_test/" + cloudRegion + "/video/720p")),
		ACL:        acl,
	}
	audioMuxingOutput := models.Output{
		OutputID:   s3OutputResp.Data.Result.ID,
		OutputPath: stringToPtr((string)("rdb_test/" + cloudRegion + "/audio")),
		ACL:        acl,
	}
	...
	manifestOutput := models.Output{
		OutputID:   s3OutputResp.Data.Result.ID,
		OutputPath: stringToPtr((string)("rdb_test/" + cloudRegion + "/manifest")),
		ACL:        acl,
	}
